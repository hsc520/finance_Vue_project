<template>
  <div class="container">
    ...
    <template v-if='hasRecord' v-for='data in datas'>
      <div class="repayList">
        <p class="tit">
        	<b class="fz28">还款日 {{data.planTime}}</b>
        	<em class="fr fz26 col6">{{data.periods}}期</em>
        </p>
        <p class="tit">
        	<em class="fz26 col6">已还款 {{data.hasRepay}}</em>
        	<b class="fr fz28">{{data.status | filterStatu}}</b>
        </p>
